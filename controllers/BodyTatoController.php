...
class BodyTatoController extends Controller
{
    ...
    function actionSimpanbody()
    {
        ...
        if(Yii::$app->request->post()){
            ...
            $cekada=BodyTato::find()->where(['no_rekam_medik'=>$_POST['no_rekam_medik']])->andWhere(['no_daftar'=>$_POST['no_daftar']]);
            ...
            if($hitungada>0){
                ...
                if ($one->save()) {
                     ...
                     return $this->asJson([
                                ...
                                'metadata'=>[
                                    'message' => "Body Tato berhasil diedit",
                                    'code' => 200,
                                ]
                            ]);
                } else {
                ...
                if ($modelnya->save()) {
                     ...
                     return $this->asJson([
                                ...
                                'metadata'=>[
                                    'message' => "Body Tato berhasil diinput",
                                    'code' => 200,
                                ]
                            ]);
                } else {
    ...
    function actionForm()
    {
            ...
            // if(isset($_GET["aksi"])){
                ...
                if(isset($_GET["id"])){
                    $id=$_GET["id"];
                    // echo $id;exit;
                    $modelupdate = BodyTato::find()->where(['no_rekam_medik'=>$id])->one();
                    // echo "<pre>";var_dump($modelupdate);exit;

                    if($modelupdate==null){
                        $DataPelayanan = DataPelayanan::find()->where(['no_rekam_medik'=>$id]);
                        $DataPelayananOne = $DataPelayanan->one();
                        $DataPelayananCount = $DataPelayanan->count();
                        if($DataPelayananCount>0){                            
                            $detailJikaBaru=["no_daftar"=>$DataPelayananOne->no_registrasi, "no_rekam_medik"=>$id];
                        }
